...
import getSessionId from "@salesforce/apex/LookupController.getSessionId";
...
export default class Lookup extends LightningElement {
  ...
  @wire(getSessionId, {})
  getSessionIdFunction({ error, data }) {
    if (error) {
  ...
  clickSearch(event) {
    if (
      !this.hasSelection &&
      (this.results.length > 0 || this.cachedResult.length > 0)
    ) {
      this.template
        .querySelector(".slds-combobox")
        .classList.add("slds-is-open");

      if (this.cachedResult.length > 0) this.results = this.cachedResult;
    }
  }
  ...
  selectItem(event) {
    ...
    this.selectedSObject = this.results.find((r) => {
      return r.objId == objId;
    });
